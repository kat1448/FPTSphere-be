...
namespace WebApi.Controllers
{
    ...
    [FromQuery] bool sortDescending = true)
        ...
        public async Task<IActionResult> ApproveEvent(int id)
        {
            ...
            {
                var userId = int.Parse(User.FindFirst(ClaimTypes.NameIdentifier)?.Value ?? "0");
                var result = await _eventService.ApproveEventAsync(id, userId);

                if (result == null)
                    return NotFound(ApiResponse<object>.ErrorResult("Event not found"));

                return Ok(ApiResponse<EventDto>.SuccessResult(result, "Event approved"));
            }
            catch (UnauthorizedAccessException ex)
            {
                return Forbid(ex.Message);
        ...
        public async Task<IActionResult> RejectEvent(int id)
        {
            ...
            {
                var userId = int.Parse(User.FindFirst(ClaimTypes.NameIdentifier)?.Value ?? "0");
                var result = await _eventService.RejectEventAsync(id, userId);

                if (result == null)
                    return NotFound(ApiResponse<object>.ErrorResult("Event not found"));

                return Ok(ApiResponse<EventDto>.SuccessResult(result, "Event rejected"));
            }
            catch (UnauthorizedAccessException ex)
            {
                return Forbid(ex.Message);
        ...
        public async Task<IActionResult> CancelEvent(int id)
        {
            try
            {
                ...
                return Ok(ApiResponse<EventDto>.SuccessResult(result, "Event cancelled"));
            }
            catch (UnauthorizedAccessException ex)
            {
                return Forbid(ex.Message);
